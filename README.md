...
:root {
  ...
  --card: #faf7f0; /* Card background */
  --accent: #7a3b10; /* Primary accent */
  --gold: #a67928; /* Gold highlights */
  --fg: #2a1e12; /* Main text */
  --fg-dim: #70604e; /* Muted text */
...
4. In the form modal's `<select id="fSubject">`, add an option:
...
| Login shows "Wrong password"         | Reset password in Firebase Console → Users → click the user → Reset Password          |
| Data not loading                     | Check Firestore Rules — `allow read: if true` must be present                         |
| Assignment links don't work          | Make sure the `path` value is correct relative to where `task-list.html` is located   |
| Console shows CORS error             | Use a local server instead of `file://` protocol                                      |
| Firebase SDK warnings in console     | These are non-blocking; they appear because compat SDK detects non-module environment |
